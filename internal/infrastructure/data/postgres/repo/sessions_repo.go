...
func (r *PostgresSessionRepositoryImpl) Update(ctx context.Context, entity *entity.Session) (*entity.Session, error) {
	query := `UPDATE sessions 
			  SET user_id = $1, ip_address = $2, device_info = $3, is_active = $4, expired_at = $5, updated_at = $6 
			  WHERE id = $7
			  RETURNING id, user_id, ip_address, device_info, is_active, expired_at, updated_at, created_at`
	row := r.database.Pool().QueryRow(ctx, query, entity.UserID(), entity.IPAddr(), entity.DeviceInfo(), entity.IsActive(), entity.ExpiredAt(), entity.UpdatedAt(), entity.ID())
	session, err := scanSession(ctx, row)
...
func (r *PostgresSessionRepositoryImpl) UpdateBySpec(ctx context.Context, spec spec.Spec) (*entity.Session, error) {
	query, args, err := spec.ToSQL()
